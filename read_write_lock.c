...
/* if there is a write lock then unlock, else if there are read locks, decrement count of read locks. If the count
 * is 0 and there is a pending writer, let it through, else if there are pending readers, let them all go through */
        pthread_mutex_lock(&(l -> read_write_lock));
                if (l -> writer > 0)
                        l -> writer = 0;
                else if (l -> readers > 0)
...
void *find_min_rw(void *list_ptr)
{
  ...
  my_min = INT_MAX;
  // mylib_rwlock_t minimum_value_lock;
...
int main(int argc, char **argv)
{
        ...
        /* init the mutex */
        mylib_rwlock_init(&minimum_value_lock);
